...
def main():
    ...
    parser.add_argument("-i", "--input", type=str, default=".data/dataframe_entity", help="A path to the prepared dataset directory.")
    parser.add_argument("-o", "--output", type=str, default=".data/dataframe_transformed", help="A path to the transformed dataset directory.")
    ...
    if not pathlib.Path(args.input).is_dir():
        logger.error(f"A prepared dataset directory path does not exist: `{args.input}`")
        sys.exit(1)
    ...
    df_games = spark.read.format("json").option("lines", "true").load(df_game_input_path)
    df_games_cleaned = clean_df(df_games, ["app_id", "date_release", "summary", "title"]).select(
    ...
    ).withColumn("discount", calculate_discount_udf(array(col("price_final"), col("price_original"))))

    logger.info(f"Cleaning `PAGE` entity dataset at `{df_page_input_path}`")

    df_pages = spark.read.format("json").option("lines", "true").load(df_page_input_path)
    ...
    ), ["app_id"]).dropna(subset=("app_id", "date_release"))

    df_game_count = df_game_joined.count()
    df_game_output_path = args.output + "/games"
    df_game_joined.coalesce(args.partitions).write.format("json")\
        .option("ignoreNullFields", False).mode("overwrite").save(df_game_output_path)

    logger.info(f"{df_game_count} `Games` entities recorded into directory `{df_game_output_path}`")
    ...
    df_review = spark.read.format("json").option("lines", "true").load(df_review_input_path)
    df_review_cleaned = clean_df(
        df_review,
        ["app_id", "found_helpful", "content", "title", "hours", "products_in_account", "miniprofile"]
    ).select(
        to_int_udf(col("app_id")).alias("app_id"),
        ...
        (col("funny") >= 0) & (col("helpful") >= 0) & (col("date").isNotNull())
    )

    df_recommendations = df_review_cleaned.select(
        col("app_id"), col("userprofile"), col("date"),
        col("is_recommended"), col("helpful"), col("funny"), col("hours")
    )
    df_recommendations_count = df_recommendations.count()
    df_recommendations_output_path = args.output + "/recommendations"
    df_recommendations.coalesce(args.partitions).write.format("json")\
        .option("ignoreNullFields", False).mode("overwrite").save(df_recommendations_output_path)

    logger.info(f"{df_recommendations_count} `Recommendations` entities recorded into directory `{df_recommendations_output_path}`")


    # Processing `Users` table stage
    df_users = clean_df(df_review_cleaned.select(col("userprofile"), col("products")), ["userprofile", "products"])
    df_users_count = df_users.count()
    df_users_output_path = args.output + "/users"
    df_users.coalesce(args.partitions).write.format("json")\
