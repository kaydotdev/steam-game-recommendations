...
def dump_table(df: DataFrame, output_dir: str, entity_name: str, fmt="csv"):
    ...
    df.coalesce(1).write.format(fmt).mode("overwrite").option("header", "true").save(
        df_output_dir
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Util to dump collected data from a PostgreSQL tables into dedicated CSV"
            " files."
        )
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=".data/dump",
        help="A path to the dump files directory.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-d",
        "--spark-db-driver-path",
        type=str,
        default="/opt/spark/jar/postgresql-42.5.3.jar",
        help=(
            "A file path to the '*.jar' file of PostgreSQL database driver for Apache"
            " Spark."
        ),
    )
    parser.add_argument(
        ...
        "--hostname",
        type=str,
        help="Database server address e.g., localhost or an IP address.",
    )
    parser.add_argument(
        "-n", "--db-name", type=str, help="The name of the PostgreSQL database."
    )
    parser.add_argument(
        "-u", "--user", type=str, help="The username used to authenticate."
    )
    parser.add_argument(
        "-p", "--password", type=str, help="Password used to authenticate."
    )
    ...
    if (
        spark_hostname is None
        or spark_db_name is None
        or spark_user is None
        or spark_password is None
    ):
        logger.error(
            "Database hostname, database name or credentials are empty. "
    ...
    spark = (
        SparkSession.builder.appName("Steam reviews dataset: Dumping records from DB")
        .master("local[*]")
    ...
    df_games = spark.read.jdbc(
        spark_jdbc_url, 'public."Games"', properties=spark_credentials
    )
    df_games_table = df_games.select(
        col("app_id"),
        replace_escape_chr_udf(col("title")).alias("title"),
        col("date_release"),
        col("win"),
        col("mac"),
        col("linux"),
    ...
    df_games_metadata = df_games.select(
        col("app_id"),
        description_udf(col("description")).alias("description"),
    ...
    df_users = spark.read.jdbc(
        spark_jdbc_url, 'public."Users"', properties=spark_credentials
    ).alias("df_users")
    df_user_reviews = (
        spark.read.format("jdbc")
        .option("url", spark_jdbc_url)
        .option(
            "query",
            (
                'SELECT "Recommendations".userprofile, COUNT(*) AS reviews FROM'
                ' "Recommendations" GROUP BY "Recommendations".userprofile'
            ),
        )
        .option("user", spark_user)
        .option("password", spark_password)
        .load()
        .alias("df_user_reviews")
    )

    df_user_with_reviews = (
        df_users.join(
            df_user_reviews,
    ...
    stIndexer = StringIndexer(
        inputCol="userprofile", outputCol="user_id", handleInvalid="error"
    )
    ...
    df_recommendations = spark.read.jdbc(
        spark_jdbc_url, 'public."Recommendations"', properties=spark_credentials
    )
    df_recommendations = profileIndexer.transform(df_recommendations).withColumn(
        "review_id", monotonically_increasing_id()
    )
    df_recommendations = df_recommendations.select(
        ...
        col("date"),
        col("is_recommended"),
        col("hours").cast(DoubleType()).alias("hours"),
        col("user_id").cast(IntegerType()).alias("user_id"),
        col("review_id"),
    )
