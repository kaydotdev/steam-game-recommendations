...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Util to load cleaned dataset into PostgreSQL database with user-password"
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Update fields for existing records or skip them.",
    )
    ...
    spark = (
        SparkSession.builder.appName("Steam reviews dataset: Data loading")
        .master("local[*]")
    ...
    df_games = (
        ...
        .option("lines", "true")
        .schema(games_schema)
        .load(df_games_input_path)
    )
    df_games_count = df_games.count()

    logger.info(f"Loading {df_games_count} `Games` entities to `{spark_jdbc_url}`")

    df_games.write.format("jdbc").option("url", spark_jdbc_url).option(
        "driver", "org.postgresql.Driver"
    ).option("dbtable", 'public."GamesAppend"').option("user", spark_user).option(
    ...
    df_recommendations = (
        ...
        .load(df_recommendations_input_path)
    )
    df_recommendations_count = df_recommendations.count()

    logger.info(
        f"Loading {df_recommendations_count} `Recommendations` entities to"
        f" `{spark_jdbc_url}`"
    )

    df_recommendations.write.format("jdbc").option("url", spark_jdbc_url).option(
        "driver", "org.postgresql.Driver"
    ).option("dbtable", 'public."RecommendationsAppend"').option(
        "user", spark_user
    ).option(
        "password", spark_password
    ).mode(
    ...
    df_users = (
        ...
        .option("lines", "true")
        .schema(users_schema)
        .load(df_users_input_path)
    )
    df_users_count = df_users.count()

    logger.info(f"Loading {df_users_count} `Users` entities to `{spark_jdbc_url}`")

    df_users.write.format("jdbc").option("url", spark_jdbc_url).option(
        "driver", "org.postgresql.Driver"
    ).option("dbtable", 'public."UsersAppend"').option("user", spark_user).option(
        "password", spark_password
    ).mode(
        "append"
    ).save()
