...
class GameswithReviewsSpider(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.only_games = (
            bool(strtobool(only_games))
            if isinstance(only_games, str)
            else bool(only_games)
        )
    ...
    def parse_search_params(self, response):
        ...
        yield scrapy.Request(
            f"https://store.steampowered.com/search/results/?query&start=0&count={self.games_per_page}&dynamic_data=&category1=998&sort_by=_ASC&snr={snr}&infinite=1&cc=us&l=english",
            cb_kwargs={"snr": snr},
        )
    ...
    def parse(self, response, **kwargs):
        ...
        self.logger.info(
            f"Crawling {pages_number} pages ({self.games_per_page} games per page,"
            f" {games_number} games in total)"
        )
        self.logger.info(
            f"Skipping {skip_pages_number} pages ({self.skip_games} games)"
        )

        for i in range(skip_pages_number, pages_number):
            yield scrapy.Request(
                (
                    f"https://store.steampowered.com/search/results/?query&start={self.games_per_page * i}&count={self.games_per_page}&dynamic_data=&category1=998&sort_by=_ASC&snr={snr}&infinite=1&cc=us&l=english"
                ),
    ...
    def parse_items(self, response, **kwargs):
        ...
        for item in Selector(text=json.loads(response.text).get("results_html")).css(
            "a"
        ):
            ...
            platforms = item.css(
                "div.responsive_search_name_combined div.search_name div"
                " span.platform_img::attr(class)"
            ).getall()
            price_final = item.css(
                "div.search_price_discount_combined::attr(data-price-final)"
            ).get()
            ...
            yield {
                ...
                "content": {
                    ...
                    "tag_ids": (
                        [
                            self.cast_or_default(x, int, default=0)
                            for x in json.loads(tag_ids)
                        ]
                    ...
                    "summary": item.css(
                        ...
                        " span.search_review_summary::attr(data-tooltip-html)"
                    ).get(),
                    "price_final": self.cast_or_default(price_final, int),
                    "price_raw": item.css(
                        "div.search_price_discount_combined div.search_price"
                    ).get(),
                    "price_discount_raw": item.css(
                        "div.search_price_discount_combined div.search_discount"
                    ).get(),
                    "steam_deck_compatible": item.css(
                        "::attr(data-ds-steam-deck-compat-handled)"
                    ).get(),
                },
            }
            ...
            if app_id is not None and not self.only_games:
                ...
                yield scrapy.Request(
                    app_page_url, cb_kwargs={"app_id": app_id}, callback=self.parse_page
                )
                yield scrapy.Request(
                    f"https://steamcommunity.com/app/{app_id}/reviews/?browsefilter=toprated&snr={snr}&filterLanguage={self.filter_language}&l=english",
    ...
    def parse_page(self, response, **kwargs):
        ...
        yield {
            ...
            "content": {
                ...
                "thumbnail": page_content.css(
                    "#gameHeaderImageCtn img.game_header_image_full::attr(src)"
                ).get(),
                "description": page_content.css(
                    "div.game_description_snippet::text"
                ).get(),
                "tags": page_content.css(
                    "#glanceCtnResponsiveRight div.popular_tags_ctn div.popular_tags"
                    " a::text"
                ).getall(),
            },
        }
    ...
    def parse_review(self, response, **kwargs):
        ...
        if next_page is not None:
            ...
            yield scrapy.Request(
                next_page_url,
                cb_kwargs={"app_id": kwargs.get("app_id")},
                callback=self.parse_review,
            )
